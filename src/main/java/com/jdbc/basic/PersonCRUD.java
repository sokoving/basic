...
public class PersonCRUD {
    ...
    // UPDATE 메서드
    public static boolean modify(String ssn, String name) {
        // 1. SQL구문을 작성
    ...
    public static List<Person> findAll() {
        ...
        try (Connection conn = Connect.makeConnection()) {
            ...
            // ResultSet을 순회해서 소비
            // next() 메서드를 통해 행들을 순서대로 지목
            while (rs.next()) {
                // getter메서드를 통해 열데이터를 추출
                String ssn = rs.getString("ssn");
                String name = rs.getString("person_name");
                int age = rs.getInt("age");

                Person person = makePerson(ssn, name, age);
